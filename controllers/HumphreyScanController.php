 ...
 */
class HumphreyScanController extends RestfulController {

  function getViewableModels() {
    return array_merge($this->getCreatableModels(), Yii::app()->params['visualfields.viewable']);
  }
  ...
  function getUpdatableModels() {
    return Yii::app()->params['visualfields.updatable'];
  }
...
//$this->_sendResponse(400, 'test', null, null);
    ...
    if (!$xml_file_exists) {
      ...
      try {
        ...
        $this->_sendResponse(400, sprintf("Error importing file: %s", $e->getMessage()), "text/html", "importHumphreyImageSet", "scan");
      }
    }
    // ELSE { // what? }
