...
$image_type = Element_OphInVisualfields_Image::getSubSpecialitySubimage($name);
...
//    $x = $file->fsScanHumphreyImage->getXPath($file, '/images');
    ...
    echo "\nimagesVfaRight[" . ($imageIndex++) . "] = \"" . $file->fsScanHumphreyImage->getPath($image_type) . $file->file_name . "\";";
  }
}
$imageIndex = 0;
foreach ($eyeLeftFilesVfa as $file) {
  // large-size image storage location:
  if ($file->fsScanHumphreyImage) {
    echo "\n imagesVfaLeft[" . ($imageIndex++) . "] = \"" . $file->fsScanHumphreyImage->getPath($image_type) . $file->file_name . "\";";
  }
}
?>
...
if (count($eyeRightFilesVfa) > 0) {
  ...
  $image_type = Element_OphInVisualfields_Image::getSubSpecialitySubimage($name);
  $width = Element_OphInVisualfields_Image::getImageWidth($image_type);
  $height = Element_OphInVisualfields_Image::getImageHeight($image_type);
  ?>
  <div id="XYZa2" class="jThumbnailScroller" style="margin-left: 100px; float:left; height:306px; width:300px; ">
    <canvas id="canvasVfaRight" class="tmp" width="<? echo $width ?>" height="<? echo $height ?>" tabindex="1"></canvas>
  </div>
  <?php
}
