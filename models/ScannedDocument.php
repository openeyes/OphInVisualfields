...
class ScannedDocument {
   ...
   * @return null
   */
  public static function getScannedDocument($category, $imageType, $assetId, $params = null) {
    $class_name = 'Element_OphIn' . ucfirst(strtolower($imageType)) . '_Document';
    $module_name = 'application.modules.OphIn' . $category
   ...
   */
  public function getScannedDocuments($category, $imageType, $pid, $params) {
    $class_name = 'Element_OphIn' . ucfirst(strtolower($imageType)) . '_Document';
    $module_name = 'application.modules.OphIn' . $category
            . '.components.' . $class_name;
   ...
   * otherwise.
   */
  public function isSupported($category, $testType) {
    $class_name = 'Element_OphIn' . ucfirst(strtolower($testType)) . '_Document';
    $module_name = 'application.modules.OphIn' . $category
            . '.components.' . $class_name;
    try {
      Yii::import($module_name, true);
